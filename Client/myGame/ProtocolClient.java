...
public class ProtocolClient extends GameConnectionClient {
    ...
    if (messageTokens.length > 0) {
      ...
      if (messageTokens[0].compareTo("join") == 0) {
        if (messageTokens[1].compareTo("success") == 0) {
          ...
          sendCreateMessage(game.getPlayerPosition(), game.getAvatarName(), game.getTextureName());
        }
      ...
      // Format: (create,remoteId,x,y,z,avatarName,textureName)
      // AND
      // Handle DETAILS_FOR message
      // Format: (dsfr,remoteId,x,y,z,avatarName,textureName)
      if (messageTokens[0].compareTo("create") == 0 || (messageTokens[0].compareTo("dsfr") == 0)) { // create a new
                                                                                                    // ghost avatar
                                                                                                    // Parse out the id
                                                                                                    // into a UUID
        String avatarName = "";
        ...
        if (messageTokens.length > 5) {
          // Parse out string textureName and ObjectName
          avatarName = messageTokens[5];
          textureName = messageTokens[6];
        }

        try {
          ghostManager.createGhostAvatar(ghostID, ghostPosition, avatarName, textureName);
        } catch (IOException e) {
      ...
      if (messageTokens[0].compareTo("wsds") == 0) {
        ...
        // Parse out the id into a UUID
        UUID ghostID = UUID.fromString(messageTokens[1]);
        sendDetailsForMessage(ghostID, game.getPlayerPosition(), game.getAvatarName(), game.getTextureName());
      }
  ...
  public void sendCreateMessage(Vector3f position, String avatarName, String textureName) {
    try {
  ...
  public void sendDetailsForMessage(UUID remoteId, Vector3f position, String avatarName, String textureName) {
    try {
